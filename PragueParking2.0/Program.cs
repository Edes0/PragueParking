...
namespace PragueParkingProgram
{
    class Program
    {
        ...
        static void Main()
        {
            // TODO: Fixa Datetime med 201 platser
            ...
            while (!exit)
            {
                Console.WriteLine(PrintBigPragueParking());
                Console.WriteLine();
        ...
        static void ParkCar()
        {
            ...
            string new_Reg = GetNewRegNumInput(Console.ReadLine());

            if (!RegPlateIsOk(new_Reg)) //---------------------------- Check if licence registration plate is correct format
            {
                PrintRegPlateFormatErrorMessage();
            }
        ...
        static void ParkMc()
        {
            ...
            if (!RegPlateIsOk(new_Reg)) //---------------------------- Check if licence registration plate is in correct format
            {
                PrintRegPlateFormatErrorMessage();
            }
            else if (TrueIfFindGetRegNumAndIndex(new_Reg, out int index)) //---------------------------- No duplicates
            {
                PrintVehicleIsAlreadyRegisteredMessage(new_Reg);
            }
            else if (!Parking.Contains(null)) //---------------------------- When parking already full
            ...
            else //---------------------------- If everything is fine so far
            {
                ...
                if (!TrueIfFindGetRegNumAndIndex(new_Reg, out index)) //---------------------------- If no possible double parking, find null instead
                {
                    for (int i = 1; i < Parking.Length; i++)
                    {
                        if (Parking[i] == null)
        ...
        static void RemoveVehicleFromParking()
        {
            ...
            try
            {
                ...
                if (TrueIfFindGetRegNumAndIndex(old_Reg, out int index)) //---------------------------- Searching for plate in Parking array
                {
                    Console.Clear();
        ...
        static void MoveIndexInParkingArray()
        {
            ...
            try
            {
                ...
                if (TrueIfFindGetRegNumAndIndex(old_Reg, out int index)) //---------------------------- Seaching for old_Reg in Parking array
                {
                    ...
                    if (parseSuccess && n != 0)
                    {
                        if (!ParkingIndexIsFull(n))
                        {
                            string vehicleType = GetVehicleType(old_Reg);

                            if (vehicleType == "-MC")
                            {
                                if (ParkingContainsOneMc(n)) // Checks if the new parking is double parking MCs.
                                {
                                    string[] vehicle = Parking[index].Split("/"); // The split breaks out old_Reg from the string
                            ...
                            PrintVehicleMovedMessage(old_Reg, n); //----------------------- Prints Vehicle moved message for all
                        }
                        else //---------------------------- Moving to a full Parking spot
                        {
                            PrintParkingIsFullMessage();
                        }
                    }
                    else //---------------------------- TryParse failed
                    {
                        PrintEnterValidParkingMessage();
                    }
                }
        ...
        static void SearchUserInterface()
        {
            Console.Clear();
            ...
            if (TrueIfFindGetRegNumAndIndex(old_Reg, out int index))
            {
        ...
        static bool TrueIfFindGetRegNumAndIndex(string registrationNumber, out int index)
        {
            for (int i = 1; i < Parking.Length; i++)
            {
                if (Parking[i] != null)
        ...
        {
            if (Parking[index] != null && Parking[index].Contains("/"))
            {
                return true;
            }
            else
        ...
        static bool RegPlateIsOk(string registrationNumber)
        {
            string invalidLetters = "ßÜÅÄÖÉÁÑÓÚ";
            ...
            bool hasNoInvalidLetter = true;

            foreach (var letter in invalidLetters)
            {
                if (registrationNumber.Contains(letter))
                {
                    return hasNoInvalidLetter = false;
                }
            }

            if (hasNumber.IsMatch(registrationNumber) && hasChar.IsMatch(registrationNumber) && hasLength && hasNoInvalidLetter)
            {
                return true;
            }
            else
        ...
        static string GetNewRegNumInput(string registrationNumber)
        {
        ...
        static string PrintBigPragueParking()
        {
...
";
        ...
        static void PrintRegPlateFormatErrorMessage()
        {
            Console.ForegroundColor = ConsoleColor.DarkRed;
            Console.Clear();
            Console.WriteLine("______________________________\n");
            Console.WriteLine("Enter a valid licence registration plate: Requirement 4-10 digits [A-Z / 0-9]\n");
            Console.WriteLine("______________________________\n");
            Console.ResetColor();
        } //------------------------------------- Prints message
        ...
        static void PrintParkingIsFullMessage()
        {
            Console.Clear();
            Console.ForegroundColor = ConsoleColor.DarkRed;
            Console.WriteLine("______________________________\n");
        ...
        static void PrintEnterValidParkingMessage()
        {
            Console.Clear();
            Console.ForegroundColor = ConsoleColor.DarkRed;
            Console.WriteLine("______________________________\n");
